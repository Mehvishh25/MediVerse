...
def main():
    ...
    # Test with sample symptoms or user input
    print("\nAvailable symptoms (some examples):")
    sample_symptoms = ['itching', 'skin_rash', 'fatigue', 'headache', 'cough', 'fever', 'nausea', 'vomiting']
    print(", ".join(sample_symptoms))
    ...
    if invalid_symptoms:
        print(f"\nWarning: Invalid symptoms detected: {invalid_symptoms}")
        print("Please use only valid symptoms from the available list.")
        return
    ...
    try:
        ...
        if wrkout:
            for i, workout_item in enumerate(wrkout, 1):
                if pd.notna(workout_item) and workout_item.strip():
                    print(f"{i}. {workout_item}")
        
        print(f"\nDIETS:")
        if die:
            for i, diet_item in enumerate(die, 1):
                if pd.notna(diet_item) and diet_item.strip():
                    print(f"{i}. {diet_item}")
