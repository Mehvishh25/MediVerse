...
# Create Blueprint
disease_bp = Blueprint('disease_predictor', __name__)
...
def helper(dis):
    ...
    try:
        ...
        pre_list = []
        if pre:
            for precaution in pre[0]:
                if pd.notna(precaution) and precaution.strip():
                    pre_list.append(precaution.strip())

        # Get medications
        med = medications[medications['Disease'] == dis]['Medication']
        med_list = [medication.strip() for medication in med.values if pd.notna(medication) and medication.strip()]

        # Get diets
        die = diets[diets['Disease'] == dis]['Diet']
        die_list = [diet.strip() for diet in die.values if pd.notna(diet) and diet.strip()]

        # Get workouts
        wrkout = workout[workout['disease'] == dis]['workout']
        wrkout_list = [w.strip() for w in wrkout.values if pd.notna(w) and w.strip()]
...
@disease_bp.route('/symptoms', methods=['GET'])
@cross_origin()
def get_symptoms():
    ...
    try:
        ...
        print(f"Error in get_symptoms endpoint: {e}")
        return jsonify({'error': 'Internal server error'}), 500
...
def get_diseases():
    ...
    try:
        ...
        print(f"Error in get_diseases endpoint: {e}")
        return jsonify({'error': 'Internal server error'}), 500